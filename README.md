...
| Moderation Service | [`dmracovit/moderation-service`](https://hub.docker.com/r/dmracovit/moderation-service) | `8085` | `DATABASE_URL` (PostgreSQL 17), `SERVICE_TOKEN`; peer URLs `APPLICANT_URL`, `CREDENTIAL_URL`, `RULES_URL`, `UNIVERSITY_RECORD_URL`, `SESSION_URL` (unset = real service, empty = built-in mock; the Postman collection needs all five mocked, see `.env.example`), `UPSTREAM_SERVICE_TOKEN` (must equal University Record's `Auth__ServiceToken`) |
